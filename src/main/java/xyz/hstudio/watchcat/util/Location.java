...
public class Location extends Vector3D {
    ...
    public Vector3D getDirection() {
        return MathUtil.getDirection(yaw, pitch);
    }
    ...
    public boolean onGround(double depth) {
        Set<BlockWrapper> blocks = new HashSet<>();
        blocks.addAll(BlockUtil.getBlocksInLocation(this));
        blocks.addAll(BlockUtil.getBlocksInLocation(this.plus(0, -1, 0)));

        AABB underFeet = new AABB(x - 0.3, y - depth, z - 0.3, x + 0.3, y, z + 0.3);

        for (BlockWrapper block : blocks) {
            if (block.isLiquid() || !BlockUtil.isSolid(block)) {
                continue;
            }
            for (AABB bBox : block.boxes()) {
